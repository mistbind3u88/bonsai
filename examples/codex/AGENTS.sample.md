...
- 単一リポジトリ内のドキュメントは、役割ごとに責務を分け、同じ説明を複数ファイルへ重複して持ち込まない方向で改善する。
- ドキュメントを更新する際は、既存文書の責務境界を確認し、転記を増やす代わりに参照関係の整理、記述場所の一本化、重複箇所の削除を優先する。
- 1 つの説明が複数文書にまたがる場合は、どの文書を基準にするかを決め、他方では要約または参照にとどめる。
