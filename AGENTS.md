...
13. **スキル AGENTS.md の整備** — 各スキルに、`templates/skill-AGENTS.md` の雛形に沿った `AGENTS.md` を配置する。`## 前提ツール`（手順が実行を前提とする外部ツール）と `## 責務の境界`（担う範囲と、担わず他スキルへ委譲する範囲）を必須セクションとし、スキル固有の補足セクションは任意で追加してよい。`CLAUDE.md` → `AGENTS.md` のシンボリックリンクは判断基準7 に従いセットで作成する
