...
2. `/doc-check` と `/rule-check` を並列で実行する。
   - どちらもサブエージェントを起動しうるため、`/check` 側では並列起動する想定でユーザーへの事前案内をまとめる。案内には、`/doc-check` と `/rule-check` の読み取り検査をサブエージェントへ並列依頼すること、依頼範囲、メインエージェントに残す判断を含める。
   - 実際の起動可否、既存サブエージェントの再利用、起動上限の余剰、依頼文の委譲境界は、各スキル内の `/subagent-check` が起動直前に判断する。`/check` はその判断を置き換えない。
   - 各スキル内の `/subagent-check` が `REUSE` / `WAIT` / `ASK_USER` / `FIX_PROMPT` を返した場合は、その判定を優先し、再利用・完了待ち・追加確認・依頼文修正を行う。
   - 同じ `/check` 実行内で修正後の再検査を行う場合は、`doc-check` と `rule-check` それぞれの既存サブエージェントを各スキル内で再利用する。役割が違うため、`doc-check` agent と `rule-check` agent を相互に流用しない。
   - 両方の検査結果をそろえてから次へ進む。
