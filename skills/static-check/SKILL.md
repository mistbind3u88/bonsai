...
検出した lint・build のコマンドは互いの結果に依存しないため、すべてバックグラウンドで並列に起動し、直列化しない。複数言語にまたがって複数のコマンドを検出した場合も、まとめて並列起動する。

全コマンドの完了を待って結果を集約する。いずれかが失敗した場合は、実行中のコマンドの完了を待ってから lint・build 全体の結果をまとめて報告する。
