...
スキル `/review-log read` を実行し、現在の branch に対応する review log から既知のレビュー判断を取得する。
...
- **共通コンテキスト**: 各プロンプトに共通で含める情報
  ...
  - **既知の論点・対応済み判断**: 以前のレビューで指摘され、対応不要と判断済みの事項があれば、指摘内容と対応不要の理由を記載する。`/review-log` から取得した既知のレビュー判断もここに含め、codex が同じ指摘を繰り返さないようにする
...
7. レビュー完了後にチェックタグを設置する

指摘がない場合、または全指摘へのユーザー判断が揃った場合は、スキル `/mark review-cross` を実行してチェックタグを設置する。
