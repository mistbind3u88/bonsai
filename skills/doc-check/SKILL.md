...
対象リポジトリにスキル管理ルールが定義されている場合は、スキル変更時の重点確認もテンプレートに含め、一覧・索引・依存図・使い分け説明が変更に追従しているかを検査させる。最小権限、スキル分類、必須セクション、エージェント向けルールファイルの symlink など、適用ルールへの適合は `/rule-check` が検査する。
...
```
ドキュメント整合性チェック:
  README.md:          新規スキルが対象リポジトリのスキル一覧に追加されていない
  docs/example.md:    散文の意図記述 — コード挙動のパラフレーズに留まっている
```
