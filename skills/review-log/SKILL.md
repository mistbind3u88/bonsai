...
- 判断理由。`対応不要` / `ループ懸念` の場合は、**検証方法と根拠を次回 reviewer が再検証できる形で残す**（コード追跡で確認した箇所、`WebSearch` で参照した公式ドキュメントの URL、実行結果など）。「設計どおり」だけで終わらせない。
...
- 対応不要またはループ懸念と判断した指摘と理由。各項目に**決定の強さ**を明示する:
  - `ユーザー裁定` の項目は「設計意図として確定。再指摘不可」と書く。reviewer は同じ論点を出さない。
...
**同一論点が複数 review で繰り返し指摘された場合は、引き継ぎでは 1 項目に統合する。** これまでに退けた角度（mechanism）を列挙し、reviewer が既出の角度を避けて新しい角度だけ出せるようにする。散発的に複数項目へ分散させない（系譜は `判断履歴` の関連 ID で追える）。
...
| ID  | Date | Source | Range | Location | Severity | Finding | Decision | Rationale | Status |
| --- | ---- | ------ | ----- | -------- | -------- | ------- | -------- | --------- | ------ |
```

`Last updated` は、実行時の会話コンテキストで示される現在日付を `YYYY-MM-DD` 形式で記録する。

`Decision` 列には決定主体を括弧書きで含める（例: `対応不要（ユーザー裁定）`、`対応不要（エージェント判断）`、`採用`）。`Rationale` 列には、`対応不要` / `ループ懸念` の場合は検証方法・参照（コード箇所・ドキュメント URL 等）を、再指摘の系譜がある場合は関連 `R0xx` を含める。
