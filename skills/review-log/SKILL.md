...
既存ログの `判断履歴` に `References` / `Related` 列がない場合は、追記前に `Status` の前へ列を追加し、既存行には空欄または `なし` を補う。既存行の `Decision` は当時の判断記録として保持し、決定主体が明示されていない値を推測で補完しない。移行後に新しい行を追加し、`Rationale` / `References` / `Related` の列意味が分かれる状態にする。
...
- 指摘の場所、深刻度、内容、根拠
- 判断（`採用` / `対応不要` / `保留` / `ループ懸念`）
- **決定主体**: `開発者` / `エージェント` のいずれか。ユーザーが明示的に採否や方針を決めた場合は `開発者`、メインエージェントがルール・差分・実行結果から採否案を確定した場合は `エージェント` として記録する。Decision セルに括弧書きで添える（例: `対応不要（開発者）`）。前回判断を引き継いだ事実は、Decision ラベルではなく `Rationale` / `References` / `Related` に記録する。
...
- 対応不要またはループ懸念と判断した指摘と理由。各項目に**決定の強さ**を明示する:
  - `開発者` の項目は「開発者判断として確定。既存判断として尊重」と書く。reviewer は新しい根拠がある別論点だけを扱う。
  - `エージェント` の項目は「新しい証拠・前提の崩れがある場合のみ再指摘可」と書き、判断の根拠（検証方法）を 1 行添えて reviewer が崩しにかかれるようにする。
- 修正済みだが同じ観点で再指摘されやすい判断
- reviewer が前提として読むべき、今回の差分に近い制約

`採用` した指摘は通常 `fixed` として `判断履歴` に残す。再指摘防止に必要な論点だけを `次回レビューへの引き継ぎ` に含める。
...
新規行の `Decision` 列には決定主体を括弧書きで含める（例: `対応不要（開発者）`、`対応不要（エージェント）`、`採用（エージェント）`）。既存行は、追記時点で確認できる当時の判断値を保持する。`Rationale` 列には判断理由を、`References` 列には検証に使ったファイルパス・URL・セクション・コマンド結果の要約を、`Related` 列には関連 `R0xx` を含める。
