...
- 既知の論点・対応済み判断。`/review-log` から取得した既知のレビュー判断があればここに含め、なければ `なし`
- hunk-level の差分
...
`claude -p` が権限・実行環境の理由で開始または完了できない場合は、失敗を呼び出し元へ報告する。fallback の有無・種別の判断は外側の workflow に委ねる。
...
- CLI 実行不可時は失敗を呼び出し元へ報告し、fallback の判断は外側の workflow に委ねる
