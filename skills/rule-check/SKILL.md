...
**このスキルはメインエージェントが実行し、読み取り検査だけを `general-purpose` サブエージェントへ依頼する。** メインエージェントは `/rule-check` の手順解釈、`/subagent-check`、起動、結果受領、最終判断、報告を担う。このスキルは、呼び出し元から起動条件を与えられる前提ではなく、自身の手順の中で `/subagent-check` を実行して起動可否・再利用・起動上限の余剰・委譲境界を主体的に判断する。必要な `mark` 設置は呼び出し元のメインエージェントが担う。
