...
**このスキルはメインエージェントが実行し、読み取り検査だけを `general-purpose` サブエージェントへ依頼する。** メインエージェントは `/rule-check` の手順解釈、`/subagent-check`、起動、結果受領、最終判断、報告を担う。必要な `mark` 設置は呼び出し元のメインエージェントが担う。
