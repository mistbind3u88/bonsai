...
| PR 全体の経過や push 後の差分を伝えたい                | `/pr-progress` に委譲する                                            |
| 既存レビューコメントへ返信したい                       | `/reply-review` に委譲する                                           |
| PR/Issue の本文を更新したい                            | `/gh-edit` に委譲する                                                |
...
- 対象行から離れた話題は `/pr-progress` または `/gh-edit` に委譲する
